...
class Mapping:
    ...
    def update_map(self, grid_map, pose, scan):
        ...
        # E-level solution below!
        robotPosition = [(pose.pose.position.x - origin.position.x), (pose.pose.position.y - origin.position.y)]
        for i in range(len(scan.ranges)):
            if scan.range_min < scan.ranges[i] and scan.range_max > scan.ranges[i]:
                laser_yaw = scan.angle_min + i * scan.angle_increment
                x = int((cos(laser_yaw + robot_yaw) * scan.ranges[i] + robotPosition[0])/resolution)
                y = int((sin(laser_yaw + robot_yaw) * scan.ranges[i] + robotPosition[1])/resolution)
